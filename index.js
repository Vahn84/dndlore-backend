      ...
      async (accessToken, refreshToken, profile, done) => {
        ...
        try {
          ...
          if (user) {
            ...
            user.googleRefreshToken = refreshToken || user.googleRefreshToken; // Keep existing if not provided
...
function requireDM(req, res, next) {
  requireAuth(req, res, () => {
    if (!req.user || req.user.role !== "DM") {
      return res.status(403).json({ error: "Forbidden: DM role required" });
    }
    next();
  });
}
...
app.put("/groups", requireDM, async (req, res) => {
  ...
  await Promise.all(
    newOrder.map((id, index) => Group.findByIdAndUpdate(id, { order: index })),
  );
  res.json({ success: true });
});
...
app.put("/groups/:id", requireDM, async (req, res) => {
  const { id } = req.params;
  const { name, color, order, exclude, orderAscending, defaultSelected } =
    req.body;
  console.log(
    "Updating group:",
    id,
...
app.post("/assets", requireDM, upload.single("file"), async (req, res) => {
  try {
    ...
    if (!url) return res.status(400).json({ error: "file or url is required" });
...
app.delete("/assets/:id", requireDM, async (req, res) => {
  try {
    ...
    if (asset.thumb_url && asset.thumb_url.startsWith("/uploads/")) {
      try {
        const p = path.join(UPLOADS_PATH, path.basename(asset.thumb_url));
        fs.unlink(p, () => {});
      } catch (e) {
        // ignore unlink errors
      }
...
Avoid meta prefaces—start in medias res.`;
    ...
    const supportsTemperature =
      /4o|gpt-4|mini|turbo/i.test(modelToUse) && !/gpt-5/i.test(modelToUse);
